    ...
    **kwargs
    ...
    if changeToPine  and peat:
        ...
        if nfi_adjustments: #Always apply close to coast and norrland for same as NFI results.
            ...
            FGRAN -= 0.000000901 * (altitude ** 2) * ~coast * limes_norrlandicus #Ordinary routine. i.e. no effect for areas close to coast north of limes norrlandicus.
        if New_LateralWater == 3:
            FGRAN += 0.09195
        if ditched:
            FGRAN += 0.13622
        ...
        FGRAN = 5.30943 - 0.01716 * (latitude - 60.0 + abs(latitude- 60.0)) - 0.0039 * (latitude - 60.0 - abs(latitude - 60.0)) - 0.000000678 * (altitude ** 2) - 0.01243 * (SoilTextureIndex ** 2)
        if New_LateralWater == 3:
            FGRAN += 0.0488
        ...
        if New_SoilDepth == deepsoil:
            FGRAN += 0.12454
        if incline_percent <= 10.0 and New_LateralWater == 1: #if incline and seldom/never lateral groundwater..
            FGRAN -= 0.06329
        if altitude >= 350 and incline_percent > 10.0 and (aspect in range(0, 113) or aspect > 337):
            FGRAN -= 0.07189
        if New_GroundLayer == 1:
            FGRAN -= 0.06842

    elif changeToPine and New_SoilMoisture == 2 and New_Vegetation <= 9:
        FGRAN = 5.34912 - 0.02037 * (latitude - 60.0 + abs(latitude - 60.0)) - 0.000000481 * (altitude ** 2)
        if New_SoilDepth == deepsoil:
            FGRAN += 0.11574
        ...
        FGRAN = 5.44789 - 0.01566 * (latitude - 60.0 + abs(latitude - 60.0)) - 0.00000102 * (altitude ** 2) - 0.0417 * SoilTextureIndex
        if New_SoilDepth == deepsoil:
            FGRAN += 0.09162
        if climate_code == 'K3':
            FGRAN += 0.12
        ...
        FGRAN = 5.46782 - 0.02013 * (latitude - 60.0 + abs(latitude - 60.0)) - 0.01074 * (SoilTextureIndex ** 2)
        if New_LateralWater in [1, 2]:
        ...
        FGRAN = 5.51735 - 0.03958 * (latitude - 60.0 + abs(latitude - 60.0)) - 0.02466 * (latitude - 60.0 - abs(latitude - 60.0))
        if New_LateralWater in [1, 2] and nfi_adjustments: #Always apply close to coast and norrland for same as NFI results. HL1979 removed coast limitation for altitude expression for MINERAL soils.
...
def NFI_SIS_PINE( 
    **kwargs
    ...
    if peat:
        ...
        if nfi_adjustments: #Always apply close to coast and norrland for same as NFI results. HL1979 removed coast limitation for altitude expression for MINERAL soils.
            FTALL -= 0.000000901 * (altitude ** 2)
        else:
            FTALL -= 0.000000901 * (altitude ** 2) * ~coast * limes_norrlandicus #Ordinary routine, i.e. no effect for areas close to coast north of limes norrlandicus.
        if New_LateralWater == 3:
            FTALL += 0.09195
        if ditched:
            FTALL += 0.13622
        ...
        if New_SoilMoisture == 2 and New_Vegetation > 9 and New_GroundLayer > 3:
            FTALL = (5.30943 - 0.01716 * (latitude - 60.0 + abs(latitude - 60.0))
...
def Hagglund_Lundmark_1979_SIS(
    ...
    if not species in ['Picea abies','Pinus sylvestris']:
        raise ValueError(f"species must be one of : 'Picea abies' or 'Pinus sylvestris', not '{species}'")
    ...
    if type(peat)!=bool and type(peat)!=np.bool_:
    ...
    if pdisna(soil_depth) and not peat:
        print('soil_depth must be one of float: NA; int 1-5. If soil_depth is NA peat must be True.')
        return np.nan
    ...
    if type(gotland)!=bool and type(gotland)!=np.bool_:
        print('Gotland must be a bool: True or False')
        return np.nan
    if type(nfi_adjustments)!= bool and type(nfi_adjustments)!=np.bool_:
        print('nfi_adjustments must be a bool: True or False')
    if type(coast)!=bool and type(coast)!=np.bool_:
        print('coast must be a bool: True or False')
    if type(limes_norrlandicus)!=bool and type(limes_norrlandicus)!=np.bool_:
