...
def test_site_index_value_creation_set(sample_age_measurement, sample_species_set, sample_fn):
    ...
    si_val = 30.5
    siv = SiteIndexValue(
        si_val, reference_age=sample_age_measurement, species=sample_species_set, fn=sample_fn
    )
...
def test_site_index_value_creation_set_multi(
    ...
    siv = SiteIndexValue(
        si_val,
        reference_age=sample_age_measurement,
        species=sample_species_set_multi,
        fn=sample_fn,
    )
    assert float(siv) == si_val
...
def test_site_index_value_creation_species_not_set(
    sample_age_measurement, sample_species, sample_fn
):
    ...
    with pytest.raises(TypeError, match="species must be a set"):
        ...
        SiteIndexValue(
            30.0, reference_age=sample_age_measurement, species=sample_species, fn=sample_fn
        )
...
def test_site_index_value_creation_species_set_wrong_type(
    sample_age_measurement, sample_species, sample_fn
):
    ...
    with pytest.raises(TypeError, match="All items in the species set must be TreeName objects"):
        SiteIndexValue(
            30.0,
            reference_age=sample_age_measurement,
            species={sample_species, "not_a_tree_name"},
            fn=sample_fn,
        )
...
def test_site_index_value_repr_set(sample_age_measurement, sample_species_set_multi, sample_fn):
    ...
    siv = SiteIndexValue(
        25.0, reference_age=sample_age_measurement, species=sample_species_set_multi, fn=sample_fn
    )
    siv_repr = repr(siv)
...
def test_site_index_value_creation(age_total_100, sample_species, sample_fn):
    ...
    si_val = 30.5
    siv = SiteIndexValue(
        si_val, reference_age=age_total_100, species={sample_species}, fn=sample_fn
    )
...
def test_site_index_reference_age_comparison(
    sample_species, sample_fn, age_total_100, age_dbh_100
):
    """Test comparisons involving siv.reference_age, checking value and code."""
    siv_total = SiteIndexValue(
        30.0, reference_age=age_total_100, species={sample_species}, fn=sample_fn
    )
    siv_dbh = SiteIndexValue(
        28.0, reference_age=age_dbh_100, species={sample_species}, fn=sample_fn
    )
