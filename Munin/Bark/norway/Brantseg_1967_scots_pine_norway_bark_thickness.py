...
def brantseg_1967_scots_pine_norway_bark_thickness(
    ...
    """
    Calculates double bark thickness for Scots Pine in South Norway based on Brantseg (1967).
    ...
    Raises:
        ValueError: If diameter_cm is negative, height_m < 1.3, or if a
    ...
    References:
        ...
        d_bark_mm = 2.9571 + 1.1499 * dbh_cm - 0.7304 * dbh_cm / height_m

    Notes:
        - Aligns with C# BarkThicknessNorway.GetBarkThickness for Pine.
    ...
    if isinstance(diameter_cm, Diameter_cm): #
        ...
        if diameter_cm.measurement_height_m != 1.3:
             warnings.warn(f"Input 'diameter_cm' (Diameter_cm) has measurement height {diameter_cm.measurement_height_m}m, but the assumes 1.3m.")
    ...
    if height_m <= 0: # Check against 0 for division safety, C# uses this check.
        ...
        return 0.0 # Return 0 if height is non-positive, matching C# implicit behavior

    # Calculate double bark thickness in mm
    double_bark_mm = (2.9571 + 1.1499 * d_cm - 0.7304 * d_cm / height_m)
