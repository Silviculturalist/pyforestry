...
class Moren_Perttu_radiation_1994:
    ...
    def _helper_1500(latitude, altitude, **kwargs):
        ...
        x4 = kwargs['x4']
        x5 = kwargs['x5']
        return x0 + x1*latitude + x2*latitude**2 + x3*latitude*altitude + x4*altitude + x5*altitude**2
    ...
    def _helper_global_radiation_avg_day_ratio(day_number, **kwargs):
        ...
        return E0 + E1*day_number + E2*day_number**2 + E3*day_number**3 + E4*day_number**4 + E5*day_number**5
    ...
    def get_gorczynski_continentality_index(self): # Renamed for clarity as a "getter"
        ...
        if self.july_avg_temp is None or self.jan_avg_temp is None:
            raise ValueError("July and January average temperatures must be provided during initialization to calculate continentality.")
        ...
        return 1.7 * (self.july_avg_temp - self.jan_avg_temp) / math.sin(math.radians(self.latitude)) - 20.4
    ...
    def calculate_temperature_sum_1000m(self, threshold_temperature):
        ...
        return Moren_Perttu_radiation_1994._helper_TS1000(self.latitude, self.altitude, **coeffs_A[threshold_temperature])
    ...
    def calculate_growing_season_duration_1000m(self, threshold_temperature):
        ...
        return Moren_Perttu_radiation_1994._helper_GS1000(self.latitude, self.altitude, **coeffs_B[threshold_temperature])

    def calculate_temperature_sum_1500m(self, threshold_temperature):
        coeffs_a = { 
            0:  {'x0': 8116.4, 'x1': -80.85,  'x2': -0.240,  'x3': 0.07762,   'x4': -5.947, 'x5': -0.000075},
            3:  {'x0': 4790.6, 'x1': -22.51,  'x2': -0.482,  'x3': 0.04992,   'x4': -4.026, 'x5': -0.000082},
            5:  {'x0': 3635.3, 'x1': -12.18,  'x2': -0.444,  'x3': 0.04041,   'x4': -3.343, 'x5': -0.000040},
            6:  {'x0': 3187.3, 'x1': -10.10,  'x2': -0.404,  'x3': 0.03691,   'x4': -3.089, 'x5':  0.000004}, # As per Table 2b [cite: 117]
            8:  {'x0': 2385.1, 'x1': -7.07,   'x2': -0.322,  'x3': 0.03101,   'x4': -2.659, 'x5':  0.000099},
            10: {'x0': 1633.3, 'x1': -2.90,   'x2': -0.260,  'x3': 0.02809,   'x4': -2.389, 'x5':  0.000173}
        }
        if threshold_temperature not in coeffs_a:
            raise ValueError(f"Invalid threshold_temperature. Available: {list(coeffs_a.keys())}")
        return Moren_Perttu_radiation_1994._helper_1500(self.latitude, self.altitude, **coeffs_a[threshold_temperature])

    def calculate_growing_season_duration_1500m(self, threshold_temperature):
        coeffs_b = { # From Table 3b [cite: 136]
            0:  {'x0': 1339.7, 'x1': -25.417, 'x2': 0.122,   'x3': 0.00978,   'x4': -0.687, 'x5':  0.000011},
            3:  {'x0': 716.5,  'x1': -8.681,  'x2': 0.005,   'x3': 0.00618,   'x4': -0.436, 'x5': -0.000013}, 
            5:  {'x0': 474.5,  'x1': -2.601,  'x2': -0.036,  'x3': 0.00376,   'x4': -0.275, 'x5': -0.000029}, # x2 corrected to -0.036 based on Eq 13b [cite: 128]
            6:  {'x0': 422.2,  'x1': -1.546,  'x2': -0.043,  'x3': 0.00327,   'x4': -0.236, 'x5': -0.000047},
            8:  {'x0': 397.1,  'x1': -2.017,  'x2': -0.034,  'x3': 0.00243,   'x4': -0.187, 'x5': -0.000045},
            10: {'x0': 349.1,  'x1': -1.928,  'x2': -0.029,  'x3': 0.00018,   'x4': -0.064, 'x5': -0.000030}
        }
        if threshold_temperature not in coeffs_b:
            raise ValueError(f"Invalid threshold_temperature. Available: {list(coeffs_b.keys())}")
        return Moren_Perttu_radiation_1994._helper_1500(self.latitude, self.altitude, **coeffs_b[threshold_temperature])
        ...
        if not selected_coeffs:
            if day_number == 366: 
                if (274, 365) in coeffs_D[region]:
                    ...
                    raise ValueError(f"Coefficient data for leap day (day 366) in region {region} not defined.")
            else:
                raise ValueError(f"Day number {day_number} out of defined ranges for region {region}.")
        return Moren_Perttu_radiation_1994._helper_global_radiation_clear_day_ratio(day_number, **selected_coeffs)

    @staticmethod
    def get_ratio_global_to_extraterrestrial_radiation_average_sky(day_number, region):
        coeffs_E = { 
            "North": {'E0': 0.338, 'E1': 0.114e-2, 'E2': 0.244e-4, 'E3': -0.316e-6, 'E4': 0.115e-8, 'E5': -0.134e-11},
        ...
        return Moren_Perttu_radiation_1994._helper_global_radiation_avg_day_ratio(day_number, **coeffs_E[region])
    ...
    def calculate_global_radiation_sum_growing_season(self, threshold_temperature):
        ...
        return Moren_Perttu_radiation_1994._helper_global_radiation_sum_growing_season(self.latitude, self.altitude, **coeffs_F[threshold_temperature])
