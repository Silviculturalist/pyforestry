...
class SwedishSite(SiteBase):
    ...
    climate_zone: Optional[Sweden.ClimateZone] = field(
    ...
    def __post_init__(self) -> None:
        ...
        sis_inputs_ok = (
            ...
            and self.field_layer is not None
            and self.bottom_layer is not None
            and self.soil_texture is not None
            and self.soil_moisture is not None
            and self.soil_depth is not None
            and self.soil_water is not None
            and self.aspect is not None
            and self.incline_percent is not None
            and self.ditched is not None
            and self.climate_zone is not None  # Check climate_zone (enum)
            and self.county is not None
            and self.n_of_limes_norrlandicus is not None
        )
        ...
        if sis_inputs_ok:
            ...
            is_peat = self.soil_texture in (
                Sweden.SoilTextureSediment.PEAT,
                Sweden.SoilTextureTill.PEAT,
            )
            is_gotland = self.county == Sweden.County.GOTLAND
            is_coast = (
                (self.distance_to_coast < 50) if self.distance_to_coast is not None else False
            )
            county_label = self.county.value.code
            climate_label = (
                self.climate_zone.value.label
            )  # Use the label string ("M1", "K2", etc.)
        ...
        if birch_inputs_ok:
            ...
            try:
                birch_si = eko_pm_2008_estimate_si_birch(
                    altitude=self.altitude,
                    latitude=self.latitude,
                    vegetation=self.field_layer,
                    ground_layer=self.bottom_layer,
                    lateral_water=self.soil_water,
