"""Regeneration quality and density (SLH) from Appendix 1: Functions for regeneration results (Elfving 2010).

Note:
Heureka's runtime uses a simplified ASINSLH/ASINW formulation instead of the
full Appendix 1 tables. In their implementation, seed trees are scaled
by /100, invarea uses 1/(plot_area + 1), invmap uses 1/(map_number + 5), wet is used instead of moist,
and the SYZ flag includes X (Gävlenborg). These discrepancies mean Appendix 1 results will not match the
Heureka runtime implementation.

Note:
Is Jonsbon actually site index scale [1-9], or MAImax according to Jonson scale? MAImax= 8*0.75(Jonson index - 2).
"""
...
class elfving_1992_regeneration:
    """SLH regeneration functions for natural and cultivated stands (Appendix 1).

    Defaults follow Appendix 1 application notes:
    - ``age_years`` = 12
    - ``n_full`` = 2_500
    ...
    Example:
        >>> site = SwedishSite(...)
        >>> jonsbon = jonson_index_from_site_index(
    ...
    def _validate_h100_input(h100_input: SiteIndexValue, main_species: TreeName) -> None:
        ...
        valid_markers = ("hagglund_1970", "Hagglund_1970", "height_trajectory")
        ...
        *,
        ...
        Args:
            ...
            county (Sweden.County | None): Swedish county enum. Used for Gotland/SYZ/T-area
                indicators and for Jonson index derivation when ``jonsbon`` is not supplied.
            soil_moisture (Sweden.SoilMoistureEnum | None): Soil moisture class. Used to set
                dry/mesic/moist indicator variables.
            jonsbon (float | None): Jonson site index (1..9). If omitted, supply
            ...
            h100_input (SiteIndexValue | None): H100 site index value used to derive Jonson
                index if ``jonsbon`` is not provided.
            ...
            n_full (float): Demanded seedlings per ha for full stocking (plants/ha).
                Default is 2_500 (Appendix 1 uses 2.5 thousand/ha).
            ...
            regen_area_ha (float): Regeneration area (ha). Used as invarea = 1/area.
            ...
            use_adjusted_coeffs (bool): Use Appendix 1 adjusted scarification coefficients.
        ...
        Raises:
            ValueError: If ``jonsbon`` is not supplied and required inputs for deriving it
                are missing, or if ``h100_input`` is not from Hagglund 1970.
        """

        def map_number_from_latitude(lat_deg: float) -> float:
            # Appendix definition: map = (X - 6050) / 50, X in km north of equator.
            return (lat_deg * 111.1 - 6050.0) / 50.0

        def age_f_from_age(age: float) -> float:
            # Appendix 1 definition: age_f = 2 * (1 / (1 + exp(-0.3 * age)) ** -0.5)
            return 2.0 * (1.0 / (1.0 + exp(-0.3 * age))** -0.5)

        def resolve_jonsbon() -> float:
            if jonsbon is not None:
                return float(jonsbon)
            if h100_input is None:
                raise ValueError("jonsbon or h100_input must be provided.")
            if main_species is None or vegetation is None or county is None:
                raise ValueError(
                    "main_species, vegetation, and county are required to derive jonsbon."
                )
            elfving_1992_regeneration._validate_h100_input(h100_input, main_species)
            return float(
        ...
        ) if soil_moisture else 0
        moist = int(
            soil_moisture in {Sweden.SoilMoistureEnum.MOIST, Sweden.SoilMoistureEnum.WET}
        ) if soil_moisture else 0
        ...
        # NOTE: Appendix 1 uses 1/area (ha). Heureka's Wikberg implementation uses
        # 1/(plot_area + 1). Keep this explicit so callers can choose behavior.
        invarea = 1.0 / regen_area_ha if regen_area_ha > 0 else 0.0

        # Appendix 1 scarification modification: 0.30 on mesic sites, 0.15 otherwise.
        scarif_coeff = 0.2692
        ...
        lp = (
            ...
            # NOTE: Appendix 1 uses seed trees per ha (no scaling). Heureka scales by /100.
            + 0.1596 * seed_trees_per_ha
            + 0.0175 * map_number_val * no_seed_trees
            + (-0.0379) * resolve_jonsbon()
            + 0.1888 * invarea
            + 0.1075 * north_flag
    ...
    def slh_cultivated(
        *,
        ...
        no_of_plants: Optional[float] = None,
        scarified: bool = False,
        burnt: bool = False,
        sown: bool = False,
        spruce: bool = False,
        ...
        Args:
            ...
            county (Sweden.County | None): Swedish county enum. Used for T-area indicator
                and for Jonson index derivation when ``jonsbon`` is not supplied.
            jonsbon (float | None): Jonson site index (1..9). If omitted, supply
                ``h100_input``, ``main_species``, ``vegetation``, and ``county`` to derive it.
            h100_input (SiteIndexValue | None): H100 site index value used to derive Jonson
                index if ``jonsbon`` is not provided.
            ...
            n_full (float): Demanded seedlings per ha for full stocking (plants/ha).
                Default is 2_500 (Appendix 1 uses 2.5 thousand/ha).
            spacing_m (float | None): Plant spacing in meters. If omitted, ``no_of_plants``
                must be provided and spacing is derived as 100/sqrt(no_of_plants).
            no_of_plants (float | None): Plants per ha for spacing derivation when
                ``spacing_m`` is not provided.
            scarified (bool): Indicator for scarification treatment.
            burnt (bool): Indicator for prescribed burning after final felling.
            sown (bool): Indicator for sown regenerations.
            spruce (bool): Indicator for spruce cultivation.
            use_adjusted_coeffs (bool): Use Appendix 1 adjusted coefficients for
        ...
        if spacing_m is None:
            if no_of_plants is None or no_of_plants <= 0:
                raise ValueError("Provide spacing_m or a positive no_of_plants.")
            spacing_val = 100.0 / sqrt(no_of_plants)
        else:
            spacing_val = spacing_m

        north_flag = int(latitude_deg > 60.0)
        map_number_val = map_number_from_latitude(latitude_deg)
        invmap = 1.0 / map_number_val if abs(map_number_val) > 1e-9 else 0.0
        t_area = int(county == Sweden.County.OREBRO)

        altxns_coeff = -0.0514
        scarif_coeff = 0.0757
        if use_adjusted_coeffs:
            # NOTE: Appendix 1 reduces altitude effect and increases scarification effect.
        ...
        invage = 1.0 / age_years if age_years > 0 else 0.0
        n_full_thousands = n_full / 1000.0

        lp = (
            3.0707
            + 0.4358 * invage
            + (-0.0614) * resolve_jonsbon()
            ...
            + (-0.0675) * int(sown)
            + 0.2597 * north_flag
            # NOTE: Appendix 1 uses invmap = 1/map. Heureka uses 1/(ky + 5).
            + 4.7901 * invmap
            + 0.2178 * t_area
            + (-0.1500) * n_full_thousands
        )
...
__all__ = ["elfving_1992_regeneration"]
