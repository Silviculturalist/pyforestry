...
class BrandelVolume:
    ...
    def get_coefficients(
        ...
        if altitude is None or fieldlayer_type is None:
            if latitude > 60:
                pine_coeff = (
                    BrandelVolume.NorthPineCoeff if over_bark else BrandelVolume.NorthPineUbCoeff
                )
                spruce_coeff = (
                    BrandelVolume.NorthSpruceCoeff
                    if over_bark
                    else BrandelVolume.NorthSpruceUbCoeff
                )
                birch_coeff = (
                    BrandelVolume.NorthBirchCoeff if over_bark else BrandelVolume.NorthBirchUbCoeff
                )
            else:
                pine_coeff = (
                    BrandelVolume.SouthPineCoeff if over_bark else BrandelVolume.SouthPineUbCoeff
                )
                spruce_coeff = (
                    BrandelVolume.SouthSpruceCoeff
                    if over_bark
                    else BrandelVolume.SouthSpruceUbCoeff
                )
                birch_coeff = (
                    BrandelVolume.SouthBirchCoeff if over_bark else BrandelVolume.SouthBirchUbCoeff
                )
            return {"Pine": pine_coeff, "Spruce": spruce_coeff, "Birch": birch_coeff}

        # Detailed mode.
        if part_of_sweden.lower() in ["south"] or (
            part_of_sweden.lower() == "middle" and latitude < 60
        ):
            if latitude < 57:
                lat_index = 0
            elif latitude < 59:
        ...
        if not over_bark:
            pine_coeff = (
                BrandelVolume.NorthPineUbCoeff
                if pine_coeff == BrandelVolume.NorthPineCoeff
                else BrandelVolume.SouthPineUbCoeff
            )
            spruce_coeff = (
                BrandelVolume.NorthSpruceUbCoeff
                if spruce_coeff == BrandelVolume.NorthSpruceCoeff
                else BrandelVolume.SouthSpruceUbCoeff
            )
            birch_coeff = (
                BrandelVolume.NorthBirchUbCoeff
                if birch_coeff == BrandelVolume.NorthBirchCoeff
                else BrandelVolume.SouthBirchUbCoeff
            )
        return {"Pine": pine_coeff, "Spruce": spruce_coeff, "Birch": birch_coeff}
    ...
    def get_volume(
        ...
        coeffs: Dict[str, List[float]] = BrandelVolume.get_coefficients(
            part_of_sweden, latitude, altitude, field_layer, over_bark
        )
