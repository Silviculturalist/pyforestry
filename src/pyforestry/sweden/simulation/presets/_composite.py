"""The composite Sweden stand-simulation workflow, shared by both pipelines.

:class:`CompositePipeline` composes published models into a deterministic,
5-year stepping pipeline (parameterized to reproduce the Heureka system's
published behaviour):
...
4. Mature-tree forecasting with **whichever single-tree growth model the subclass
   supplies**, including any stand-level correction that model brings.
5. Standing valuation using Söderberg (1992) bark/height and Mellanskog (2013) prices.

Only step 4 varies. :mod:`.elfving_2010_pipeline` fills it with Elfving (2010)
and its stand basal-area calibration; :mod:`.soderberg_1986_pipeline` with
Söderberg (1986), which has no such calibration.

This module is where the two used to share code by one inheriting from the
other: ``Soderberg1986Pipeline`` subclassed ``Elfving2010Pipeline``, so one
published model's runner was the base class of another's, the Elfving model was
constructed and discarded on every Söderberg run, and the Söderberg module
imported two private names across the boundary. The eleven phases were always
the shared thing, so they live here and neither pipeline is the other's parent.
"""
...
# ``SPRUCE_SET`` does match, and the Söderberg pipeline imports it from here rather
...
def _to_ingrowth_species(species: TreeName | None) -> IngrowthSpeciesGroup:
    ...
    if species in PINE_SET:
        return IngrowthSpeciesGroup.PINE
    if species in SPRUCE_SET:
        return IngrowthSpeciesGroup.SPRUCE
    if species in BIRCH_SET:
...
class CompositePeriodRecord:
    ...
    left the previous period's figures standing and :meth:`CompositePipeline._snapshot_row`
...
class _BeginPeriodStep(_PipelineStep):
    """Open a fresh :class:`CompositePeriodRecord` for the period.
    ...
    def run(self, ctx: SimulationContext, dt: float) -> None:
        ...
        self.pipeline._record = CompositePeriodRecord()
...
class CompositePipeline:
    ...
    One period is the eleven phases in :meth:`_build_steps`: reconstruct or grow
    the young stand with Nyström (2000), predict mortality, run a mature
    single-tree growth model, blend the two diameter trajectories, realise
    mortality, age, recruit with Wikberg (2004), and refresh heights and bark with
    Söderberg (1992). Around it sit the NYSKOG initialisation and the valuation.

    Everything above is shared. What a subclass supplies is the mature growth
    model and its provenance -- :meth:`_build_model`, :meth:`component_id`,
    :meth:`source`, :meth:`components` -- plus how that model takes its inputs
    (:meth:`_model_inputs` for a model with a typed ``Inputs``,
    :meth:`_model_attrs` for one that reads ``ctx.attrs``).

    This class used to be ``Elfving2010Pipeline``, and ``Soderberg1986Pipeline``
    inherited from it: one published model's runner was the base class of
    another's. It built an ``Elfving2010Model`` that the subclass immediately
    ...
    * **Broadleaves are tapered as pine** -- :meth:`_pulpwood_volume_m3fub`, this
    ...
    def _build_model(self) -> Any:
        """Return the mature single-tree growth model this pipeline steps with.

        Called once, from ``__init__``, before anything reads ``self._model``.

        Raises:
            NotImplementedError: Always; a composite pipeline is defined by which
                mature model it drives, so there is no default worth having.
        """
        raise NotImplementedError(
            f"{type(self).__name__} must implement _build_model() and return the "
            "mature growth model it steps with. The composite workflow around it -- "
            "NYSKOG, Nyström, mortality, ingrowth, height and bark -- is inherited."
        )

    @property
    def component_id(self) -> str:
        """Stable identifier for this composite pipeline.

        Raises:
            NotImplementedError: Always; a subclass names itself.
        """
        raise NotImplementedError(f"{type(self).__name__} must declare a component_id.")

    @property
    def source(self) -> SourceReference:
        """Bibliographic provenance for the model this pipeline projects with.

        Raises:
            NotImplementedError: Always. The default used to be Elfving (2010),
                inherited by whichever pipeline forgot to override it -- a citation
                that reads as real wherever provenance is reported.
        """
        raise NotImplementedError(
            f"{type(self).__name__} must declare a source: the publication behind the "
            "mature growth model it drives. The composite itself is a pyforestry "
            "composition and cites nothing of its own."
        )
    ...
    def _model_inputs(self) -> Any | None:
        ...
        Override in a subclass whose model declares an ``Inputs`` type, and return
        an instance of it. These used to be string keys written onto ``ctx.attrs``
        after the context was built, where a mistyped name was indistinguishable
        from a site that had none of that value and only surfaced as a default --
        or an exception -- once a growth kernel reached for it.

        Returns:
            ``None`` by default, which is right for a model that reads
    ...
    def _model_attrs(self) -> Mapping[str, Any]:
        ...
        Empty by default, which is what a model with a typed ``Inputs`` needs. A
        model that predates that contract -- Söderberg (1986) -- overrides this
        instead, and which of the two a subclass fills in is the honest record of
        which contract its model is on.
        """
        return {}
    ...
    def _stand_structure(self) -> dict[str, Any]:
        ...
        site_index_dominant_m = (
            ...
            if dominant_species in SPRUCE_SET
...
__all__ = [
    # Declared because ``CompositePipeline.steps`` is now data a caller can add to,
    # and a step they write has to name the type it reports into.
    "CompositePeriodRecord",
