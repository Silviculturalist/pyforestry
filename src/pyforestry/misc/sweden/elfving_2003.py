...
from pyforestry.site.sweden.sweden_site_primitives import Vegetation
from pyforestry.helpers.tree_species import TreeSpecies, TreeName, parse_tree_species, RegionalGenusGroup
...
class Lind2003MeanDiameterCodominantTrees:
    ...
    @staticmethod
    def estimate(*, total_stand_age: Number, stand_density_gf: Number, site_index: Number) -> float:
        ...
        ln_md = (Lind2003MeanDiameterCodominantTrees._C0 +
        ...
        md = exp(ln_md)
        if md <= 0: raise ArithmeticError("Calculated dominant-mean diameter is non-positive.")
        return md
...
class _ModelParams: # Simplified for brevity, ensure all fields from previous version are here
    ...
    is_uneven_aged_override: Optional[bool] = None; is_standard_override: Optional[bool] = None
    is_undergrowth_override: Optional[bool] = None; dominant_mean_diameter_override: Optional[float] = None
    is_gotland_override: Optional[bool] = None; is_ditched_override: Optional[bool] = None
    is_peat_soil_override: Optional[bool] = None; is_shade_tolerant_broadleaf_override: Optional[bool] = None
    is_contorta_pine_flag: int = field(init=False)
    log_d_cm: float = field(init=False); dominant_mean_diameter: Optional[float] = field(init=False)
    diameter_qmd_ratio: Optional[float] = field(init=False); dfrel_ratio: Optional[float] = field(init=False)
    ...
    inverted_sis: Optional[float] = field(init=False); temperature_sum: float = field(init=False)
    is_rich_site_flag: int = field(init=False); is_poor_site_flag: int = field(init=False)
    is_spruce_flag: int = field(init=False); is_pine_flag: int = field(init=False)
    is_light_decid_flag: int = field(init=False); is_bokek_flag: int = field(init=False)
    is_shade_tolerant_broadleaf_flag: int = field(init=False)
    is_gotland_flag: int = field(init=False); is_ditched_flag: int = field(init=False)
    is_peat_soil_flag: int = field(init=False)
    is_standard_tree: bool = field(init=False); is_undergrowth_tree: bool = field(init=False)
    is_ost_flag: int = field(init=False); is_sma_flag: int = field(init=False)

    def __post_init__(self):
        if self.d_cm <= 0: raise ValueError("diameter (d_cm) must be > 0 cm")
        self.log_d_cm = log(self.d_cm)
        self.temperature_sum = 0.001 * (4835.0 - 5.76 * self.latitude - 0.9 * self.altitude_m)
        self.is_contorta_pine_flag = 1 if self.species_obj == TreeSpecies.Sweden.pinus_contorta else 0

        if self.sis is not None:
            self.inverted_sis = 1.0 / self.sis if self.sis > 0 else None
        else: self.inverted_sis = None
        ...
        elif (self.processed_total_stand_age is not None and self.sis is not None and self.sis > 0 and
              self.processed_basal_area_relascope_m2_ha is not None and self.processed_basal_area_relascope_m2_ha > 0):
            try:
                self.dominant_mean_diameter = Lind2003MeanDiameterCodominantTrees.estimate(
                    total_stand_age=self.processed_total_stand_age, site_index=self.sis,
                    stand_density_gf=self.processed_basal_area_relascope_m2_ha)
            except (ValueError, ArithmeticError): self.dominant_mean_diameter = self.d_cm 
        else: self.dominant_mean_diameter = self.d_cm

        if self.dominant_mean_diameter and self.dominant_mean_diameter > 0:
            self.dfrel_ratio = self.d_cm / self.dominant_mean_diameter
        else: self.dfrel_ratio = None 

        if self.local_qmd_cm and self.local_qmd_cm > 0:
            self.diameter_qmd_ratio = self.d_cm / self.local_qmd_cm
        else: self.diameter_qmd_ratio = None 
        ...
        else: self.log_processed_basal_area_plot = None
        ...
        else: self.log_inverted_relascope_ba_plus_1 = None
        ...
        light_decid_species = (TreeSpecies.Sweden.ulmus_glabra, TreeSpecies.Sweden.tilia_cordata,
                               TreeSpecies.Sweden.acer_platanoides, TreeSpecies.Sweden.prunus_avium)
        self.is_light_decid_flag = 1 if self.species_obj in light_decid_species or \
                                   (hasattr(TreeSpecies.Sweden, 'salix') and isinstance(TreeSpecies.Sweden.salix, RegionalGenusGroup) and self.species_obj in TreeSpecies.Sweden.salix) else 0
        self.is_bokek_flag = 1 if self.species_obj == TreeSpecies.Sweden.fagus_sylvatica or \
                               (hasattr(TreeSpecies.Sweden, 'quercus') and isinstance(TreeSpecies.Sweden.quercus, RegionalGenusGroup) and self.species_obj in TreeSpecies.Sweden.quercus) else 0
        
        shade_tolerant_broadleaves = (TreeSpecies.Sweden.carpinus_betulus, TreeSpecies.Sweden.sorbus_aucuparia)
        ...
        else: self.is_standard_tree = self.is_undergrowth_tree = False
...
class Elfving2003SingleTreeAge:
    _BIAS = {2:0.028,3:0.062,4:0.042,5:0.012,6:0.018,7:0.038,8:0.034,9:0.045, 51:0.012}
    ...
    _COEFF_2_TOTAL_W_AGE = (-1.46921, #const
    ...
    _COEFF_5_PINE = ( # C# CoefficientPine: const, d, logD, d/Dm, bald, logBald, logG
        -1.69566, -0.00801, 0.29819, 0.07258, -0.00076986, 1.15873, 0.03098
    )
    _COEFF_6_SPRUCE = ( # C# CoefficientSpruce: const, d, logD, d/QMD, logBald, logG
        -1.38343, -0.01003, 0.37897, 0.12834, 1.02011, 0.01748
    )
    _COEFF_7_BIRCH = ( # C# CoefficientBirch: const, d, logD, d/QMD, logBald, log(Gf+1), SIS, rich
        0.10055, -0.00793, 0.52067, -0.08139, 0.67013, 0.03945, -0.01053, -0.09905
    )
    ...
    _COEFF_9_STD = ( # C# CoeffFunction5: const, d, logD, bald, sis, ljuslov
        1.79933, -0.01841, 1.14894, 0.00634, -0.03460, -0.22537
    )
    _COEFFS = {2:_COEFF_2_TOTAL_W_AGE,3:_COEFF_3_TOTAL_WO_AGE,4:_COEFF_4_UNEVEN,5:_COEFF_5_PINE,
                 6:_COEFF_6_SPRUCE,7:_COEFF_7_BIRCH,8:_COEFF_8_DECID,9:_COEFF_9_STD,
                 51:_COEFF_5_PINE} 
    _CalculatorFunc = Callable[[_ModelParams, Tuple[float, ...]], float]

    @staticmethod
    def _prepare_model_params(diameter:Number,species_input:str|TreeName,total_stand_age:Optional[Number],SIS_spruce:Optional[Number],field_layer:Any,basal_area_plot_m2_ha:Optional[Number],basal_area_relascope_m2_ha:Optional[Number],altitude_m:Number,latitude:Number,QMD_cm:Optional[Number],stems_ha:Optional[int],is_uneven_aged_override:Optional[bool],dominant_mean_diameter_override:Optional[Number],is_standard_override:Optional[bool],is_undergrowth_override:Optional[bool],is_gotland_override:Optional[bool],is_ditched_override:Optional[bool],is_peat_soil_override:Optional[bool],is_shade_tolerant_broadleaf_override:Optional[bool]) -> _ModelParams:
        species_obj = parse_tree_species(species_input) # type: ignore
        if not isinstance(species_obj, TreeName): raise TypeError(f"parse_tree_species expected TreeName, got {type(species_obj)}") # type: ignore
        
        # CORRECTED: No 0.01 scaling for basal_area_plot_m2_ha
        processed_ba_plot = float(basal_area_plot_m2_ha) if basal_area_plot_m2_ha is not None else None
        if processed_ba_plot is not None and processed_ba_plot < 0: raise ValueError("Plot BA cannot be negative.")

        processed_tsa = min(max(float(total_stand_age),10.0),175.0) if total_stand_age is not None else None
        
        processed_ba_relascope = float(basal_area_relascope_m2_ha) if basal_area_relascope_m2_ha is not None else processed_ba_plot # Fallback
        if processed_ba_relascope is not None and processed_ba_relascope < 0: raise ValueError("Relascope BA cannot be negative.")
    ...
    def _determine_calculation_group(params: _ModelParams) -> int:
        if params.is_standard_override if params.is_standard_override is not None else params.is_standard_tree: return 9
        ...
        if params.processed_total_stand_age is None: return 3 # No age, not Contorta (already checked)
        ...
        if hasattr(TreeSpecies.Sweden, 'pinus') and isinstance(TreeSpecies.Sweden.pinus, RegionalGenusGroup) and \
           species in TreeSpecies.Sweden.pinus and not params.is_contorta_pine_flag: # type: ignore
            is_pine_non_contorta_or_larch = True
        elif hasattr(TreeSpecies.Sweden, 'larix') and isinstance(TreeSpecies.Sweden.larix, RegionalGenusGroup) and \
             species in TreeSpecies.Sweden.larix: # type: ignore
            is_pine_non_contorta_or_larch = True
        if is_pine_non_contorta_or_larch: return 5
        
        if species == TreeSpecies.Sweden.picea_abies: return 6 # type: ignore
        if hasattr(TreeSpecies.Sweden, 'betula') and isinstance(TreeSpecies.Sweden.betula, RegionalGenusGroup) and species in TreeSpecies.Sweden.betula: return 7 # type: ignore
        
        # CORRECTED: Access tree_type as a property
        if species.tree_type == "Deciduous": return 8 # type: ignore
    ...
    def _calculate_ln_a13_group_pine_contorta(p: _ModelParams, c: Tuple[float, ...]) -> float:
        ...
        adjusted_bald_val = p.processed_total_stand_age - 2.0 if p.processed_total_stand_age is not None else None
        ...
        if p.dominant_mean_diameter_override is None: # Only recalculate if Dm was not an override
            if (adjusted_bald_val is not None and adjusted_bald_val > 1E-5 and
                adjusted_sis_val is not None and adjusted_sis_val > 0 and
                p.processed_basal_area_relascope_m2_ha is not None and
                p.processed_basal_area_relascope_m2_ha > 0):
                try:
                ...
                except (ValueError, ArithmeticError): pass # Keep previously calculated p.dominant_mean_diameter
        
        dfrel_for_formula = p.d_cm / dm_for_contorta if dm_for_contorta and dm_for_contorta > 0 else None

        # Determine bald and logBald for formula
        bald_for_formula = p.processed_total_stand_age # Default to original
        log_bald_for_formula = log(p.processed_total_stand_age) if p.processed_total_stand_age and p.processed_total_stand_age > 1E-5 else None
        ...
        if bald_for_formula is None or bald_for_formula <=0 : raise ValueError("Contorta: bald_for_formula is invalid.")
        if dfrel_for_formula is None : raise ValueError("Contorta: dfrel_for_formula is invalid.")
        if p.log_processed_basal_area_plot is None : raise ValueError("Contorta: log_processed_basal_area_plot (ln g) is invalid.")
        if log_bald_for_formula is None: raise ValueError("Contorta: log_bald_for_formula is invalid.")

        # Pine formula using (potentially) adjusted values
        # c: const, d, logD, d/Dm, bald, logBald, logG
        return (c[0] + c[1]*p.d_cm + c[2]*p.log_d_cm + c[3]*dfrel_for_formula +
                c[4]*bald_for_formula + c[5]*log_bald_for_formula +
    ...
    def _calculate_ln_a13_group4(p: _ModelParams, c: Tuple[float, ...]) -> float: 
        ...
        return (c[0] +                                     # const
                ...
                c[16]*p.is_gotland_flag + # CORRECTED from c[17] * p.is_gotland_flag, C# uses c[16] for gotland
                c[17]*p.is_ditched_flag                     # dike
               )
    @staticmethod
    ...
    @staticmethod
    def _calculate_ln_a13_group8(p: _ModelParams, c: Tuple[float, ...]) -> float: # Other Deciduous
    ...
    def _calculate_ln_a13_group9(p: _ModelParams, c: Tuple[float, ...]) -> float: # Standards
        ...
        if p.processed_total_stand_age is None : raise ValueError("G9: total_stand_age is required.") 
    ...
    _GROUP_CALCULATORS = {2:_calculate_ln_a13_group2,3:_calculate_ln_a13_group3,4:_calculate_ln_a13_group4,
                          5:_calculate_ln_a13_group5,6:_calculate_ln_a13_group6,7:_calculate_ln_a13_group7,
    ...
    def age(*, diameter:Number,species:str|TreeName,total_stand_age:Optional[Number]=None,SIS:Optional[Number]=None,field_layer:Any=Sweden.FieldLayer.BILBERRY,basal_area_plot_m2_ha:Optional[Number]=None,basal_area_relascope_m2_ha:Optional[Number]=None,altitude_m:Number=100,latitude:Number=64,QMD_cm:Optional[Number]=None,stems_ha:Optional[int]=None,is_uneven_aged:Optional[bool]=None,dominant_mean_diameter:Optional[Number]=None,is_standard_tree_hint:Optional[bool]=None,is_undergrowth_tree_hint:Optional[bool]=None,is_gotland:Optional[bool]=None,is_ditched:Optional[bool]=None,is_peat_soil:Optional[bool]=None,is_shade_tolerant_broadleaf_hint:Optional[bool]=None) -> float: # type: ignore
        ...
        coeffs,bias,calculator = Elfving2003SingleTreeAge._COEFFS[group], Elfving2003SingleTreeAge._BIAS[group], Elfving2003SingleTreeAge._GROUP_CALCULATORS.get(group)
        if calculator is None: raise NotImplementedError(f"Calculator for group {group} undefined.")
        ln_a13 = calculator(params,coeffs)+bias; a13 = exp(ln_a13)
        if not (0<a13<1000): raise ArithmeticError(f"Age {a13:.2f} (group {group}) implausible.")
        return a13
    ...
    @staticmethod
    def _is_rich_site(fi:Any)->int:co=Elfving2003SingleTreeAge._get_vegetation_code(fi);return 1 if co and(1<=co<=9 or co==12)else 0
    @staticmethod
    def _is_poor_site(fi:Any)->int:co=Elfving2003SingleTreeAge._get_vegetation_code(fi);return 1 if co and co>13 else 0
